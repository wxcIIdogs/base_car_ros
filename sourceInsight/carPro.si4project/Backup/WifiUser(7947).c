...
{
	static int s_scanTime = 10;
	//Wifi.MyGateWay
  if(Wifi.TcpIpConnections[0].status != WifiConnectionStatus_Connected)
  {
	  if(Wifi_TcpIp_StartTcpConnection(0,"192.168.0.102",8888,10))
	  {
	  ...
	  else
	  {
		  ...
		  if(Wifi_TcpIp_Ping("192.168.0.102"))
			  printf("ip %s to ping 192.168.0.102 success\r\n",Wifi.MyIP);
		  else
			  printf("ip %s to ping 192.168.0.102 false\r\n",Wifi.MyIP);
	  }
  }
  else
  {
